...
const Recipes = ({ selectedOption }) => {
  ...
  useEffect(() => {
  if (selectedOption) {
    const searchRecipe = JSON.parse(selectedOption);
    setRecipeIndex(searchRecipe.id - 1);
  }
  }, [selectedOption]);
  ...
  return (
    <>
      ...
      >
        ...
        {!loading && <span>Shuffle</span>}
      </button>
      {loading && (
        <span className="loading loading-infinity m-auto w-52 text-emerald-500"></span>
      )}
      {!loading && (
        <div className="border border-gray rounded-lg p-4 w-10/12 md:w-3/12 gap-4 flex flex-col m-auto shadow-xl mt-4">
          ...
          <div className="flex flex-wrap">
            ...
            {recipe.duration && (
              <p className="py-2 px-3 bg-gray-200 rounded-md text-xs mr-2 mb-2">
                {recipe.duration}
              </p>
            )}
          </div>
